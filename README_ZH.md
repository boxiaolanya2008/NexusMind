...
flowchart TB
    ...
    subgraph Backend["后端 (Express.js)"]
        ...
        Services --> Cache["缓存服务"]
        Services --> KB["知识库"]
        Services --> Memory["内存系统"]
        Services --> Format["格式转换"]
    end
...
**技术说明**：
- 当前使用 SQLite 作为数据库，适合中小规模部署
- 请求队列在内存中实现，服务重启会丢失队列数据
- 无分布式缓存层，建议生产环境添加 Redis
- 单进程部署，无高可用设计
...
- **数据库**：SQLite 适合中小规模，高并发场景建议迁移至 PostgreSQL
- **缓存**：建议添加 Redis 缓存层提升性能
- **队列**：当前内存队列会丢失数据，生产环境建议使用 BullMQ + Redis
- **CORS**：配置 CORS 白名单，不要使用 `origin: '*'`
- **密钥**：定期轮换 JWT_SECRET，使用强随机密钥
- **监控**：建议集成监控告警系统（如 Prometheus + Grafana）
